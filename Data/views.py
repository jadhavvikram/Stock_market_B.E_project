...
def about(request):
    CSV_URL = 'https://www.alphavantage.co/query?function=IPO_CALENDAR&apikey=apikey=BP39EYKNTFWF2FOF'
    results = []
    with requests.Session() as s:
        ...
        for row in my_list:
            # print(row)
            results.append(row)
    return render(request, 'about.html', {'results': results})
...
def fundamentals(request):
    ...
    try:
        # Fetch all scrip codes
        all_codes = b.getScripCodes()
        print("All Codes:", all_codes)  # Debug: Print all codes

        # Find the scrip code for the search value
        scripCode = get_key_by_value(all_codes, search)
        print("Scrip Code:", scripCode)  # Debug: Print scrip code

        if scripCode is None:
            return render(request, 'fundamentals.html', {'error': 'Scrip code not found for the given search value.'})

        # Fetch the quote for the scrip code
        q = b.getQuote(scripCode=scripCode)
        print("Quote Data:", q)  # Debug: Print quote data

        return render(request, 'fundamentals.html', {'api': q})
    except Exception as e:
        print("Error:", e)  # Debug: Print any errors
        return render(request, 'fundamentals.html', {'error': '--------------'})
...
def holidays(request):
    ...
    api_request = requests.get("https://www.nseindia.com/api/holiday-master?type=trading", headers=headers)
    try:
        print(api_request)
        api = json.loads(api_request.content)
        return render(request, 'holidays.html', {'data':api})
    except Exception as e:
        return render(request, 'holidays.html', {'error':e})
...
def results(request):
    ...
    if request.method == 'POST':
        ...
        st_dis_name = search
        print(search)
        request_p = search.split(" - ")[0].rpartition('.')[0]
        print(request_p)
        company_name = search.split(" - ")[1].split("--->")[0]
        # request_p = search
        ex = '.NS'
        st_name = request_p + ex
        print(st_name)
        try:
            stock_data = yf.Ticker(st_name)
            # Get quarterly financial data
            quarterly_financials = stock_data.quarterly_financials
            return render(request, 'results.html', {'data':quarterly_financials.to_html(), 'name': company_name})
        except Exception as e:
            return render(request, 'results.html', {'error':e})
        
    return render(request, 'results.html')

def options(request):
    if request.method == 'POST':
        search = request.POST['search3']
        print(search)
        try:
            headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
            api_request = requests.get("https://www.nseindia.com/api/option-chain-indices?symbol=" + search, headers=headers)
            api = json.loads(api_request.content)
            data = {"options":[]}
            # print(api["records"]["data"])
            for input_json in api["records"]["data"]:
                if "PE" in input_json:
                    first_key = "PE"
                else:
                    first_key = "CE"
                ...
                # print(input_json[first_key])
                # print(input_json[second_key])
                # count = 0
                for key in input_json[first_key]:
                    # print("line 136")
                    # print(count)
                    # count += 1
                    if key in input_json[second_key]:
                        
                        if input_json[first_key][key] == input_json[second_key][key]:
                            merged_data[key] = input_json[first_key][key]
                        else:
                            merged_data[key] = f"{input_json[first_key][key]}/{input_json[second_key][key]}"
                data['options'].append(merged_data)   
                keys = merged_data.keys()
                print("keys")
            return render(request, 'options.html', {'data':data, 'keys': keys})
        except Exception as e:
            print(e)
            return render(request, 'options.html', {'error':e})
        
    return render(request, 'options.html')

def fiidii(request):
    return render(request, 'fiidii.html')
...
def ipos(request):
    ...
    api_request = requests.get("https://www.nseindia.com/api/ipo-current-issue", headers=headers)
    try:
        print(api_request)
        api = json.loads(api_request.content)
        return render(request, 'ipos.html', {'ipoData':api})
    except Exception as e:
        return render(request, 'ipos.html', {'error':e})
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        search = search.split(" - ")[0].rpartition('.')[0]
